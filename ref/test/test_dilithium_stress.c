...
static int run_client_once(const char *ip, const uint8_t *sk,
                           uint32_t *challenge_len_out,
                           size_t *sig_len_out,
    ...
    if (challenge_len_out) {
        *challenge_len_out = challenge_len;
    }
    if (sig_len_out) {
        *sig_len_out = sig_len;
    }
    if (elapsed_ms_out) {
        *elapsed_ms_out = get_time_ms() - start_ms;
    }
    ...
    while (batches == 0 || batch <= batches) {
        ...
        for (spawned = 0; spawned < concurrent; ++spawned) {
            ...
            if (pid == 0) {
                ...
                int status = run_client_once(ip, sk, &challenge_len, &sig_len, &elapsed_ms);
                log_result(log_path, status, challenge_len, sig_len, elapsed_ms);
                _exit(status);
            }
